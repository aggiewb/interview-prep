...
function pairwise(arr, arg) {
    ...
    const indices = [];
    arr.forEach((num, i) => {
      const remainder = arg - num;
      let remainderIndex = arr.indexOf(remainder);
      if(remainderIndex > -1 && i !== remainderIndex){
        if(!indices.includes(i) && !indices.includes(remainderIndex)){
          indices.push(i, remainderIndex);
          arr.splice(remainderIndex, 1, null);
        }
      }
    });
    const indicesTotal = indices.reduce((accumulator, num) => num + accumulator);
    return indicesTotal;
}